...
public class MainController {
    ...
    @FXML private Button btnAllNotes;
    @FXML private Button btnFavorites;
    @FXML private Button btnShared;
    @FXML private Button btnTrash;
    @FXML private Button btnProductLaunch;
    @FXML private Button btnClientSyncs;

    // ===== MIDDLE PANEL CONTROLS =====
    @FXML private Button btnFilterAll;
    @FXML private Button btnFilterRegular;
    @FXML private Button btnFilterDone;
    @FXML private Button btnFilterUrgent;
    @FXML private Button btnFilterIdeas;
    @FXML private ComboBox<String> sortComboBox;
    @FXML private VBox vboxNotesList;
    @FXML private javafx.scene.layout.FlowPane notesContainer;
    @FXML private Label lblNotesCount;
    @FXML private Label noteCountLabel;

    // ===== RIGHT PANEL CONTROLS =====
    @FXML private Label lblNoteTitle;
    @FXML private Label lblNoteDate;
    @FXML private Label lblNoteTime;
    @FXML private Label lblNoteStatus;
    @FXML private TextArea txtContent;
    @FXML private Button btnFavoriteNote;
    @FXML private Button btnDeleteNote;
    @FXML private Button btnEditTitle;
    ...
    private void loadAllNotes() {
        ...
        System.out.println("loadAllNotes: Loading notes for user ID = " + currentUser.getId() + " (" + currentUser.getUsername() + ")");
    ...
    private void createSampleNotes() {
        ...
        // Create sample notes with different statuses - không set CategoryID để tránh FK error
        Note note1 = new Note();
        note1.setUserId(currentUser.getId());
        note1.setTitle("Q4 Roadmap Finalization");
        note1.setContent("Need to sync with the design team regarding the final assets for the mobile release...\n\n📋 Tasks:\n• Review design mockups\n• Schedule meeting with team\n• Prepare presentation slides");
        ...
        note3.setContent("Exploring how we can use natural language processing to automatically categorize incoming notes...\n\n💡 Ideas:\n• Auto-tagging based on content analysis\n• Smart search with semantic understanding\n• Sentiment analysis for meeting notes\n• Automatic summary generation");
        ...
        note4.setContent("Finalized the headings for the home page and the features section...\n\n✅ Completed:\n• Hero section copy\n• Feature descriptions\n• Call-to-action buttons\n• Footer content");
    ...
    private void filterAndDisplayNotes() {
        ...
        System.out.println("filterAndDisplayNotes: currentFilter=" + currentFilter + ", showFavoritesOnly=" + showFavoritesOnly + ", currentSort=" + currentSort);
    ...
    private void displayNotesInList(List<Note> notes) {
        ...
        for (Note note : notes) {
            System.out.println("displayNotesInList: Creating card for note ID=" + note.getId() + ", title=" + note.getTitle());
    ...
    private void displayNoteInEditor(Note note) {
        if (note == null) {
            if (lblNoteTitle != null) lblNoteTitle.setText("");
            if (lblNoteDate != null) lblNoteDate.setText("");
            if (lblNoteTime != null) lblNoteTime.setText("");
            if (lblNoteStatus != null) lblNoteStatus.setText("");
        ...
        if (dt != null) {
            if (lblNoteDate != null) lblNoteDate.setText(DATE_FORMAT.format(dt));
            if (lblNoteTime != null) lblNoteTime.setText(TIME_FORMAT.format(dt).toUpperCase());
        }
        
        // Display status badge
        if (lblNoteStatus != null) {
            lblNoteStatus.setText(note.getStatus().toUpperCase());
            lblNoteStatus.getStyleClass().removeAll("status-regular", "status-urgent", "status-ideas", "status-completed");
    ...
    private void setNavButtonActive(Button activeBtn) {
        ...
        if (btnTrash != null) btnTrash.getStyleClass().remove("nav-btn-active");
    ...
    private void handleFormatBold() {
        if (txtContent == null || selectedNote == null) return;
        wrapSelectedText("**", "**");
    }
     ...
     * This provides a Notion-like tab/panel editing experience instead of popup dialogs.
     */
     ...
     * Displays title input, color picker, favorite checkbox, and create/cancel buttons.
     */
    private void showCreatePanel() {
        ...
        editorPanel.getChildren().removeIf(node -> 
            node instanceof VBox && "createPanel".equals(node.getId())
        );
        ...
        txtNewTitle.setStyle("-fx-font-size: 16px; -fx-padding: 10; -fx-background-radius: 8; -fx-border-color: #cbd5e1; -fx-border-radius: 8;");
        ...
        btnCreate.setStyle("-fx-background-color: #3b82f6; -fx-text-fill: white; -fx-font-weight: bold; -fx-padding: 10 30; -fx-background-radius: 8; -fx-cursor: hand;");
        btnCreate.setOnAction(e -> handleConfirmCreate());
        
        Button btnCancel = new Button("Hủy");
        btnCancel.setStyle("-fx-background-color: #e2e8f0; -fx-text-fill: #475569; -fx-padding: 10 30; -fx-background-radius: 8; -fx-cursor: hand;");
    ...
    private HBox createColorPicker() {
        ...
        for (int i = 0; i < colors.length; i++) {
            ...
            colorBtn.setStyle("-fx-background-color: " + color + "; -fx-background-radius: 20; -fx-cursor: hand; -fx-border-width: " + borderStyle + "; -fx-border-color: " + borderColor + "; -fx-border-radius: 20;");
            ...
            colorBtn.setOnAction(e -> {
                ...
                colorBox.getChildren().forEach(node -> {
                    if (node instanceof Button) {
                        ...
                        btn.setStyle("-fx-background-color: " + btnColor + "; -fx-background-radius: 20; -fx-cursor: hand; -fx-border-width: " + 
    ...
    private void handleConfirmCreate() {
        ...
        // Validation
        if (title.isEmpty()) {
            txtNewTitle.setStyle("-fx-border-color: #fc8181; -fx-border-width: 2; -fx-border-radius: 8; -fx-background-radius: 8;");
     ...
     * Returns to view mode without saving (with confirmation if user has typed content).
     */
