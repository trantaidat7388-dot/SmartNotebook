...
public class NoteServiceV2 {
    ...
    public Note createNote(String title, String content, String htmlContent) {
        ...
        if (title == null || title.trim().isEmpty()) {
            ...
            String textContent = htmlContent != null ? 
                SmartTextUtil.stripHtml(htmlContent) : content;
            title = SmartTextUtil.suggestTitle(textContent);
        }
        note.setTitle(title);
        ...
        String textContent = htmlContent != null ? 
            SmartTextUtil.stripHtml(htmlContent) : content;
        note.setSummary(SmartTextUtil.generateSummary(textContent));
    ...
    public Note createNote(Note note) {
        ...
        if (note.getTitle() == null || note.getTitle().trim().isEmpty()) {
            String content = note.getHtmlContent() != null ? 
                SmartTextUtil.stripHtml(note.getHtmlContent()) : note.getContent();
            note.setTitle(SmartTextUtil.suggestTitle(content));
        }
        
        // Smart feature: Auto-generate summary
        String content = note.getHtmlContent() != null ? 
            SmartTextUtil.stripHtml(note.getHtmlContent()) : note.getContent();
        note.setSummary(SmartTextUtil.generateSummary(content));
    ...
    public boolean updateNoteContent(int noteId, String title, String content, String htmlContent) {
        ...
        String textContent = htmlContent != null ? 
            SmartTextUtil.stripHtml(htmlContent) : content;
        String summary = SmartTextUtil.generateSummary(textContent);
